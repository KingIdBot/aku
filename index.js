...
const { limits } = require('./Fxc7/limit')
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:MrKing\n'
            + 'ORG:Creator MrKing;\n'
            + 'TEL;type=CELL;type=VOICE;waid=18313535216:+1 (831) 353-5216\n'
            + 'END:VCARD'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FarhanGans = ["18313535216@s.whatsapp.net"]
			...
			mess = {
				...
				only: {
					...
					userB: `ʜᴀɪ *${pushname2}* ᴋᴀᴍᴜ ʙᴇʟᴜᴍ ᴍᴇɴᴊᴀᴅɪ ᴛᴇᴍᴀɴ *MR KING BOT* \nꜱɪʟᴀʜᴋᴀɴ ᴅᴀꜰᴛᴀʀ ᴅᴜʟᴜ yᴀᴀ ᴅᴇɴɢᴀɴ ᴄᴀʀᴀ\n\n ᴋᴇᴛɪᴋ ${prefix}ᴅᴀꜰᴛᴀʀ ${pushname2}/17/jakarta`,
			...
			const ownerNumber = [
			"18313535216@s.whatsapp.net","6282246586611@s.whatsapp.net"
			]
			premium = [
	        "18313535216@s.whatsapp.net","6281290121665@s.whatsapp.net","62814622392081@s.whatsapp.net","6289502020754@s.whatsapp.net","62 83144780782@s.whatsapp.net","6282246586611@s.whatsapp.net"
			]
		...
		case 'daftar':
			...
			case 'menu':
                ...
                setTimeout( () => {
					...
					setTimeout( () => {
					client.sendMessage(from, 'Sebelum Gunakan Bot Alangkah Baiknya Baca rulesnya dan penggunaannya dulu', text, {quoted: mek})
					}, 0)
    				break
...
//akhir kreator
		    ...
		    case 'darkjokes':
				...
				reply(mess.wait)
                 ...
                 client.sendMessage(from, darkjokes, image, {quoted: mek, caption: '\`\`\`MR KING JOKES\`\`\`'})
				await limitAdd(sender) 
				break  
			case 'katailham':
				 if (isBanned) return reply(mess.only.benned) 
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 MR KING BOT 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 MR KING BOT 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat MrKingBot\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *MR KING BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak *${pushname2}*, Command *${prefix}${command}* Tidak Terdaftar Di Dalam *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[MRKINGBOT]','red'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
