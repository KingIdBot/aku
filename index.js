...
* Author MhankBarBar, Farhan,MrKing
	...
	client.on('chat-update', async (mek) => {
     ...
     const bayarLimit = (sender, amount) => {
		...
		case 'daftar':
					...
					if (isBanned) return reply(mess.only.benned)
					if (args.length < 1) return reply(`Parameter Salah\nCommand : ${prefix}daftar nama/umur\nContoh : ${prefix}daftar nama/umur/Asal Kota`)
					reg = `${body.slice(8)}`
					jeneng = reg.split("/")[0];
					umure = reg.split("/")[1];
					asal = reg.split("/")[2];
			...
			case 'menu':
                ...
                setTimeout( () => {
					costum(help(prefix, instagram, yt, name, pushname2, user, limitt), text, MrKingBot, replySet)
					}, 15000)
					setTimeout( () => {
...
//akhir kreator
		    ...
		    case 'map':
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat MR KING BOT\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *MRKING BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
